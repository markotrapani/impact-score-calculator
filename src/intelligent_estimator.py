...
class IntelligentImpactEstimator:
    ...
    def estimate_rca_action_item(self) -> Tuple[int, str]:
        ...
        IMPORTANT:
        - 8 points: Ticket is a follow-up action item from a PAST RCA
        - 0 points: Ticket IS the current incident (even if requesting RCA)
        ...
        # Check for indicators this is CURRENT incident needing RCA (score 0)
        current_incident_indicators = [
            'need rca', 'requires rca', 'requesting rca', 'rca needed',
            'investigate', 'under investigation', 'root cause unknown',
            'need to determine', 'need to find'
        ]

        if any(indicator in combined for indicator in current_incident_indicators):
            reasons.append("Current incident requiring RCA investigation (0 points)")
            return 0, '; '.join(reasons)

        # Check for indicators this is PAST RCA follow-up action item (score 8)
        action_item_indicators = [
            'action item from', 'rca recommendation', 'follow-up from',
            'identified in rca', 'per rca', 'as per rca', 'rca action',
            'implement', 'add monitoring', 'add validation', 'improve'
        ]

        # Check if RCA field has substantial content suggesting completed RCA
        if rca_text and rca_text.strip() and len(rca_text.strip()) > 100:
            # Check if it mentions action items or improvements
            if any(indicator in rca_text.lower() for indicator in action_item_indicators):
                reasons.append("Action item from completed RCA")
                return 8, '; '.join(reasons)
            else:
                # Long RCA content but unclear if action item or current incident
                reasons.append("RCA field contains content, but unclear if action item or current incident")
                return 0, '; '.join(reasons)

        # Check for explicit action item keywords
        if any(indicator in combined for indicator in action_item_indicators):
            reasons.append("Follow-up action item from past RCA")
            return 8, '; '.join(reasons)

        # Check labels
        labels_str = ' '.join(self.ticket_data.get('labels', [])).lower()
        if 'rca-action-item' in labels_str or 'action-item' in labels_str:
            reasons.append("RCA action item label found")
            return 8, '; '.join(reasons)

        reasons.append("No RCA action item indicators found (current incident or no RCA)")
        return 0, '; '.join(reasons)
